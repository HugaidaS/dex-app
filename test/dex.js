...
contract("Dex", (accounts) => {
  ...
  it("should create market order & match", async () => {
    ...
    assert(balances[0].toString() === web3.utils.toWei("50")); // -50 DAI for buying 5 REP
    assert(balances[1].toString() === web3.utils.toWei("5")); // Now I have 5 REP
    assert(balances[2].toString() === web3.utils.toWei("50")); // +50 DAI for selling 5 REP
    assert(balances[3].toString() === web3.utils.toWei("95")); // And I have just sold 5 REP, 95 REP left
  });
  ...
  it.only("should NOT create market order if token does not not exist", async () => {
    await expectRevert(
